...
report 50113 GetSalesOrdersNew
{
    ...
    dataset
    {
        dataitem("Sales Line"; "Sales Line")
        {
            ...
            begin
                ...
                if salesHeaderRec.Get("Sales Line"."Document Type", "Sales Line"."Document No.") then begin
                    if SalesHeaderStatus = SalesHeaderStatus::Released then begin
                        if salesHeaderRec.Status <> salesHeaderRec.Status::Released then
                            CurrReport.Skip();
                    end else if SalesHeaderStatus = SalesHeaderStatus::Open then begin
                        if salesHeaderRec.Status <> salesHeaderRec.Status::Open then
                            CurrReport.Skip();
                    end;
                end else
                    CurrReport.Skip();
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                group("Sales Order")
                {
                    field(SalesHeaderStatus; SalesHeaderStatus)
                    {
                        ApplicationArea = all;
    ...
    var
        ...
        SalesHeaderStatus: Option Released,Open;
        HideDialog: Boolean;
